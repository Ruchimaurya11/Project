...
public class PlanDao {
	public static void addPlan(Plan p) {
		try {
			...
			String sql = "insert into plan(wp_id,budget,style,noofguest,venue,photosvideos,noofguestfortransport) values(?,?,?,?,?,?,?)";
			PreparedStatement pst = conn.prepareStatement(sql);
			pst.setInt(1,p.getwp_id() );
			pst.setString(2, p.getBudget());
			pst.setString(3,p.getStyle());
			pst.setInt(4,p.getNoofguest());
			pst.setString(5, p.getVenue());
			pst.setString(6, p.getPhotovideos());
	...
	public static List<Plan> getPlanByWPId(int id) {
		...
		try {
			...
			while(rs.next()) {
				...
				p.setPhotovideos(rs.getString("photovideos"));
